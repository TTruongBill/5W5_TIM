arrows = document.querySelectorAll(".left-right button");//var contenant les flèches gauche droite des carrousels
enseignants = document.getElementsByClassName("enseignants");//var contenant la liste des enseignants(un par un) - 14 objets(profs)
nbRandom = Math.floor(Math.random() * 14);//nombre aléatoire pour générer un prof différent à chaque refresh(pour format desktop)
counter = document.getElementById("counter");//Var contenant le compteur du nombre de prof
imgCarrousel = document.getElementsByClassName("imgProf");//var contenant la liste des images des enseignants(un par un) - 14 objets(images)

nbProf = nbRandom;//nb pour incrémenter le prof dans lequel on est rendu
